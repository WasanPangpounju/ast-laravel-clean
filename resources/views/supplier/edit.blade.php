 ...
 <div class="content-wrapper">
	   <div class="">
		   <ol class="breadcrumb">
				...
				<li class="breadcrumb-item"><a href="#">ข้อมูลซัพพลายเออร์</a></li>
				<li class="breadcrumb-item active">แก้ไขข้อมูลซัพพลายเออร์</li>
			</ol>
	   </div>
		...
		<div class="content">
		    <div class="box-from">
			  ...
			  <form method="post" action="{{ route('supplier.update' , $dataEdit->id) }}">
			  @csrf
			@method('PATCH')
			  <div class="form-group">
				   <label for= "name">ชื่อบริษัท *</label>
				   <input type="text" class="form-control" id="name" name="name" placeholder="ชื่อบริษัท" value="{{ $dataEdit->name }}" required >
			  </div>
			  <div class="form-group">
			  ...
			  <div class="col-md-6">
				  <div class="form-group">
					...
					@if($dataEdit->email  == 'Empty Your Email')
...
@endif
			  ...
			  <fieldset>
				...
				<?php 
				 $cuscoorData = App\Http\Controllers\SupplierController::coordinatorData( $dataEdit->tax );
				?>
            <table class="table table-striped" id="dynamicAddRemove">
					...
					@if(count($cuscoorData) == null)
<?php $i =0; ?>
<tr>
	<td><input type="text" name="coordinator[{{ $i}}][name]" placeholder="ผู้ประสานงาน" class="form-control" value="" required />
	</td>
	<td><input type="text" name="coordinator[{{$i }}][jobTitle]" placeholder="ตำแหน่ง" class="form-control" value=""/>
	</td>
	<td><input type="text" name="coordinator[{{ $i }}][tel]" placeholder="เบอร์โทรศัพท์" class="form-control" value=""/>
	</td>
...
<!-- 
					@foreach($cuscoorData as $coo) 
						<input type="hidden" name="coordinator[0][coID]" value="{{ $coo->id }}">
					<tr>
						<td><input type="text" name="coordinator[0][name]" placeholder="ผู้ประสานงาน" class="form-control" value="{{ $coo->name }}" required />
						</td>
						<td><input type="text" name="coordinator[0][jobTitle]" placeholder="ตำแหน่ง" class="form-control" value="{{ $coo->jobTitle }}" />
						</td>
						<td><input type="text" name="coordinator[0][tel]" placeholder="เบอร์โทรศัพท์" class="form-control" value="{{ $coo->tel }}" />
						</td>
						<td><button type="button" name="add" id="dynamic-ar" class="btn btn-outline-primary">Add</button></td>
                	</tr>
				@endforeach -->
