            ...
            <div class="box-from">
                <div class="">
                    ...
                    <form method="post" action="{{ route('materialstore.store') }}">
                        @csrf
                        <div class="line_btn">
                            <button type="submit" class="btn btn-etc"> <i class="fas fa-pencil-alt"></i>
                                แก้ไขข้อมูลรายการเบิกวัตถุดิบ</button>
                            <button type="submit" class="btn btn-etc"> <i class="fas fa-folder"></i>
                                ดูข้อมูลเบิกวัตถุดิบทั้งหมด</button>
                        </div>
                                        ...
                                        <tbody>
                                            @foreach ($withdrawmaterial as $withdrawmaterial1)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <form method="POST"
                                                            action="{{ route('materialoutside.destroy', $withdrawmaterial1->id) }}">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button class="btn btn-danger" type="button"
                                                                onclick="if(confirm('ต้องการลบหรือไม่?')) { this.form.submit(); }">ลบ</button>
                                                            <button class="btn b_order" type="button"><a
                                                                    href="{{ route('materialoutside.edit', $withdrawmaterial1->id) }}">แก้ไข</a></button>
                                                        </form>
                                                    </td>
                                                </tr>
                                            @endforeach
                                    ...
                                    <table class="table table-bordered table-a">
                                        ...
                                        <tbody>
                                            ...
                                            @foreach ($lastTenRecords as $withdrawmaterial1)
                                                <tr>
                                                    <td>{{ $withdrawmaterial1->supplierName }}</td>
                                                    <td>{{ $withdrawmaterial1->recipient }}</td>
                                                    <td>{{ $withdrawmaterial1->yarnType }}</td>
                                                    <td>{{ $withdrawmaterial1->comment }}</td>
